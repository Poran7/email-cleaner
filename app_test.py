...
<style>
  :root {
    --bg:#040810; --surface:#080f1a; --card:#0b1623; --border:#0f2a40;
    --cyan:#00f5ff; --green:#00ff88; --orange:#ff6b00; --red:#ff2d55;
    --yellow:#ffd60a; --blue:#2979ff; --purple:#bf5af2;
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .logo {
    ...
    font-size:clamp(20px,5vw,40px); color:var(--cyan);
    text-shadow:0 0 20px rgba(0,245,255,.6),0 0 60px rgba(0,245,255,.3);
    letter-spacing:3px; animation:flicker 4s infinite;
  }
  @keyframes flicker { 0%,95%,100%{opacity:1} 96%{opacity:.7} 97%{opacity:1} 98%{opacity:.8} }
  .sub { font-size:14px; color:var(--dim); margin-top:8px; letter-spacing:2px; text-transform:uppercase; }
  .badges { display:flex; gap:10px; justify-content:center; flex-wrap:wrap; margin-top:14px; }
  .url-badge {
    display:inline-block; background:rgba(0,245,255,.07);
    border:1px solid rgba(0,245,255,.25); border-radius:6px;
    padding:6px 18px; font-family:'Share Tech Mono',monospace;
    font-size:13px; color:var(--cyan);
  }
  .tg-badge {
    display:inline-block; background:rgba(0,170,255,.1);
    border:1px solid rgba(0,170,255,.35); border-radius:6px;
    padding:6px 18px; font-family:'Share Tech Mono',monospace;
    font-size:13px; color:#00aaff; text-decoration:none; transition:all .3s;
  }
  .tg-badge:hover { background:rgba(0,170,255,.2); }
  .card {
  ...
  .dropzone {
    ...
    padding:50px 20px; text-align:center; cursor:pointer; transition:all .3s; position:relative;
  }
  ...
  .dropzone .drop-title { font-size:20px; font-weight:700; color:var(--text); margin-bottom:8px; }
  .dropzone .drop-sub { font-size:14px; color:var(--dim); }
  .dropzone input[type=file] { position:absolute; inset:0; opacity:0; cursor:pointer; width:100%; height:100%; }
  .file-chosen { margin-top:14px; font-size:14px; color:var(--green); font-family:'Share Tech Mono',monospace; display:none; }
  .btn {
    ...
    border:none; border-radius:8px; cursor:pointer; margin-top:18px; transition:all .3s;
  }
  .btn:hover { box-shadow:0 0 30px rgba(0,245,255,.5); transform:translateY(-2px); }
  ...
  .progress-bar-fill {
    ...
    width:0%; transition:width .4s ease; box-shadow:0 0 10px rgba(0,245,255,.5);
  }
  .progress-label { font-size:13px; color:var(--dim); font-family:'Share Tech Mono',monospace; margin-top:8px; text-align:center; }
  ...
  .stat-item {
    ...
    border-radius:10px; padding:16px 12px; text-align:center; transition:transform .2s;
  }
  .stat-item:hover { transform:translateY(-3px); }
  .stat-num { font-family:'Share Tech Mono',monospace; font-size:28px; font-weight:700; display:block; margin-bottom:4px; }
  ...
  .dl-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:10px; }
  .dl-btn {
    display:flex; align-items:center; gap:8px; padding:12px 16px;
    border-radius:8px; border:1px solid; font-family:'Rajdhani',sans-serif;
    font-size:14px; font-weight:600; cursor:pointer; text-decoration:none;
    transition:all .25s; background:transparent; letter-spacing:1px;
  }
  .dl-btn:hover { transform:translateY(-2px); }
  .dl-all {
    width:100%; justify-content:center; padding:14px; font-size:16px;
    letter-spacing:2px; margin-bottom:12px;
    border-color:var(--yellow); color:var(--yellow);
  }
  .dl-all:hover { background:rgba(255,214,10,.08); box-shadow:0 0 20px rgba(255,214,10,.2); }
  .tg-card {
    text-align:center; border-color:rgba(0,136,204,.4) !important;
  ...
  .tg-btn:hover { box-shadow:0 0 40px rgba(0,170,255,.6); transform:translateY(-2px); }
  .scanlines {
    position:fixed; inset:0; pointer-events:none; z-index:999;
    background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,.08) 2px,rgba(0,0,0,.08) 4px);
  }
  .footer { text-align:center; margin-top:40px; font-size:12px; color:var(--dim); font-family:'Share Tech Mono',monospace; letter-spacing:2px; }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.4} }
  .pulse { animation:pulse 1.5s infinite; }
  .c-green{color:#00ff88} .c-red{color:#ff2d55} .c-cyan{color:#00f5ff} .c-dim{color:#4a6a80}
</style>
</head>
<body>
...
<div class="wrap">
  ...
  <div class="header">
    ...
    <div class="badges">
      <div class="url-badge">🌐 zeroshell-cleaner.up.railway.app</div>
      <a class="tg-badge" href="https://t.me/ZeroShell_Store" target="_blank">✈️ @ZeroShell_Store</a>
    </div>
  </div>

  <div class="card">
    <div class="card-title">📤 ফাইল আপলোড করুন</div>
    <form id="uploadForm">
      <div class="dropzone" id="dropzone">
        <input type="file" id="fileInput" name="file" accept=".txt">
        <span class="icon">📂</span>
        <div class="drop-title">এখানে .txt ফাইল ছাড়ুন</div>
        <div class="drop-sub">অথবা ক্লিক করে ফাইল বেছে নিন</div>
        <div class="file-chosen" id="fileChosen">✅ ফাইল নির্বাচিত: <span id="fileName"></span></div>
      </div>
      <div id="progress-wrap">
        <div class="progress-bar-bg"><div class="progress-bar-fill" id="progressBar"></div></div>
        <div class="progress-label" id="progressLabel">⏳ প্রসেস হচ্ছে...</div>
      </div>
      <button type="submit" class="btn" id="cleanBtn">
        <span>🔥</span> CLEAN & SEPARATE
      </button>
    </form>
  </div>
  ...
  <div class="card" id="stats-wrap">
    ...
    <div class="card-title" style="margin-top:10px">💾 ফাইল ডাউনলোড করুন</div>
    <a class="dl-btn dl-all" id="dlAll" href="#" download>📦 সব একসাথে ডাউনলোড (ZIP)</a>
    <div class="dl-grid" id="dlGrid"></div>
  </div>

  <div class="card tg-card">
    <div style="font-size:40px;margin-bottom:12px;">✈️</div>
    <div style="font-family:'Share Tech Mono',monospace;font-size:24px;color:#00aaff;
                text-shadow:0 0 20px rgba(0,170,255,.6);margin-bottom:8px;letter-spacing:3px;">
      ZeroShell Store
    </div>
    <div style="color:#4a6a80;font-size:13px;letter-spacing:2px;margin-bottom:22px;">
      TOOLS · SERVICES · SUPPORT
    </div>
    <a class="tg-btn" href="https://t.me/ZeroShell_Store" target="_blank">
      ✈️ &nbsp;JOIN TELEGRAM
    </a>
    <div style="margin-top:16px;font-size:13px;color:#4a6a80;">
      💬 DM for support &nbsp;·&nbsp;
      <a href="https://t.me/ZeroShell_Store" target="_blank" style="color:#00aaff;text-decoration:none;">
        @ZeroShell_Store
      </a>
    </div>
  </div>
  ...
  <div class="footer">
    SMART EMAIL CLEANER v2.0 · PYTHON + FLASK<br>
...
document.getElementById('uploadForm').addEventListener('submit',async e=>{
  ...
  if(!fi.files.length){alert('⚠️ প্রথমে একটি .txt ফাইল বেছে নিন!');return;}
  const btn=document.getElementById('cleanBtn');
  btn.disabled=true; btn.innerHTML='<span class="pulse">⏳</span> প্রসেস হচ্ছে...';
  ...
  const iv=setInterval(()=>{
    ...
    pb.style.width=pct+'%';
    pl.textContent=`⚙️ বিশ্লেষণ হচ্ছে... ${Math.round(pct)}%`;
  },120);
  ...
  try {
    ...
    clearInterval(iv);
    if(data.error){alert('❌ '+data.error);btn.disabled=false;btn.innerHTML='🔥 CLEAN & SEPARATE';return;}
    pb.style.width='100%'; pl.textContent='✅ সম্পন্ন!';
    setTimeout(()=>{pw.style.display='none';showStats(data);},600);
  } catch(err){clearInterval(iv);alert('❌ '+err.message);}
  btn.disabled=false; btn.innerHTML='<span>🔥</span> CLEAN & SEPARATE';
});
...
function showStats(data){
  ...
  document.getElementById('statsGrid').innerHTML=`
    <div class="stat-item"><span class="stat-num c-green">${total}</span><span class="stat-label">✅ মোট ইউনিক</span></div>
    <div class="stat-item"><span class="stat-num c-red">${data.duplicates}</span><span class="stat-label">❌ ডুপ্লিকেট</span></div>
    <div class="stat-item"><span class="stat-num c-cyan">${data.total_lines}</span><span class="stat-label">📄 মোট লাইন</span></div>
    <div class="stat-item"><span class="stat-num c-dim">${data.invalid}</span><span class="stat-label">🗑️ ইনভ্যালিড</span></div>`;
  const cw=document.getElementById('chartWrap');
  const order=['gmail','hotmail','outlook','live','msn','other'];
  const maxVal=Math.max(...order.map(k=>cats[k]||0),1);
  cw.innerHTML=order.map(k=>{
    const c=COLORS[k]; const v=cats[k]||0;
    ...
    return `<div class="chart-row">
      ...
      <div class="chart-bar-bg">
        <div class="chart-bar-fill" style="background:${c.color};box-shadow:0 0 8px ${c.color}40" data-width="${pct}%"></div>
      </div></div>`;
  }).join('');
  setTimeout(()=>document.querySelectorAll('.chart-bar-fill').forEach(el=>el.style.width=el.dataset.width),100);
  document.getElementById('dlAll').href='/download/all?session='+data.session;
  document.getElementById('dlGrid').innerHTML=order.filter(k=>cats[k]>0).map(k=>{
    const c=COLORS[k];
    return `<a class="dl-btn" href="/download/${k}?session=${data.session}" download
              ...
              ${c.icon} ${k.toUpperCase()} <span style="opacity:.6;font-size:12px">(${cats[k]})</span></a>`;
  }).join('');
}
</script>
</body>
...
def clean():
    if 'file' not in request.files:
        return jsonify({'error': 'ফাইল পাওয়া যায়নি'}), 400
    f = request.files['file']
    if not f.filename.endswith('.txt'):
        return jsonify({'error': 'শুধু .txt ফাইল দিন'}), 400
    ...
    if cat == 'all':
        ...
        with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            for k,(title,domain) in cfg.items():
                if cats.get(k):
                    zf.writestr(f"{k}_unique.txt", make_file_content(cats[k],title,domain))
            all_accs = []
            for k in ['gmail','hotmail','outlook','live','msn','other']:
                all_accs.extend(cats.get(k,[]))
            zf.writestr("ALL_unique.txt", make_file_content(all_accs,"ALL ACCOUNTS - UNIQUE","All domains"))
        buf.seek(0)
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        return send_file(buf, as_attachment=True,
                         download_name=f"cleaned_{ts}.zip",
...
if __name__ == '__main__':
    ...
    print(f"\n{'═'*50}")
    print(f"  🔥  SMART EMAIL CLEANER STARTING...")
